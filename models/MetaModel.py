...
def tta_meta_minimax(meta_model, train_data, lr, epoch, args, engine, mode):
    ...
    running_loss, running_corrects = AverageMeterDict(), AverageMeterDict()
    meta_model.train()
    inner_opt_max, params_max = get_new_optimizers(meta_model, opt_type=args.inneropt, lr=args.meta_lr, names=['bn'], layers=args.max_bn_layer,
...
def tta_meta_minimax1(meta_model, train_data, lr, epoch, args, engine, mode):
    ...
    running_loss, running_corrects = AverageMeterDict(), AverageMeterDict()
    meta_model.train()
    inner_opt_max, params = get_new_optimizers(meta_model, opt_type=args.inneropt, lr=args.meta_lr, names=['bn'],
                                               layers=args.max_bn_layer,
                                               param_names=['bias'], momentum=args.meta_second_order)
    inner_opt_min, _ = get_new_optimizers(meta_model, opt_type=args.inneropt, lr=args.meta_lr, names=['bn'],
                                          nonpara_name=params,
                                          lambd_lr=args.meta_lambd_lr, param_names=args.inner,
...
def tta_meta_minimax_test(meta_model, eval_data, lr, epoch, args, engine, mode):
    ...
    inner_opt_max, params = get_new_optimizers(meta_model, opt_type=args.inneropt, lr=args.meta_lr, names=['bn'],
                                               layers=args.max_bn_layer,
                                               param_names=['bias'], momentum=args.meta_second_order)
    inner_opt_min, _ = get_new_optimizers(fast_model, opt_type=args.inneropt, lr=args.meta_lr, names=['bn'],
                                          nonpara_name=params,
                                          lambd_lr=args.meta_lambd_lr, param_names=args.inner,
                                          momentum=args.meta_second_order)
    step = 0
